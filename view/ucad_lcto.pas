...
begin
  ...
  lancamento.conta    := StrToIntDef(edtConta.text,0);
  lancamento.cod_plano:= StrToIntDef(edtCodPlano.text,0);
  lancamento.data_mvto:= edtData.Date;
  if (StrToIntDef(edtCodPlano.Text,0) = 99990) and
     (StrToIntDef(edtContaDestino.Text,0) > 0) then
     lancamento.descricao:= 'Destino: '+edtContaDestino.Text+' '+
                             edtDescLcto.text
  else
     lancamento.descricao:= edtDescLcto.text;


  if plano.tipo = 'C' then
     lancamento.valor    := edtValor.Value
  else
    lancamento.valor     := (edtValor.Value*-1);
  if lancamento.inclui then
     begin
       if (StrToIntDef(edtCodPlano.Text,0) = 99990) and
          (StrToIntDef(edtContaDestino.Text,0) > 0) then
           begin
             lancamento.conta    := StrToIntDef(edtContaDestino.text,0);
             lancamento.cod_plano:= 99991;
             lancamento.data_mvto:= edtData.Date;
             lancamento.descricao:= 'Conta:'+inttostr(lancamento.conta)+' '+
                                    'Id:'+inttostr(lancamento.id_lcto)+' '+
                                    edtDescLcto.text;

             lancamento.valor    := edtValor.Value;
             if not lancamento.inclui then
...
begin
  if (strtointdef(edtContaDestino.Text,0) > 0) and
     (strtointdef(edtContaDestino.Text,0) <> StrToInt(edtConta.Text)) then
     begin
       if ContaDestino.localiza(StrToIntDef(edtContaDestino.Text,0)) then
          begin
            edtDescDestino.text := ContaDestino.descricao;
            edtDescLcto.SetFocus;
          end
       else
          begin
     ...
     begin
      ShowMessage('Código destino inávlido'+sLineBreak+
      'conta destino não pode ser igual conta de origem');
      edtCodPlano.SetFocus;
     end;
...
begin
  if Assigned(plano) then
     FreeAndNil(plano);
  if Assigned(ContaDestino) then
     FreeAndNil(ContaDestino);
...
begin
  lancamento.conta    := StrToIntDef(edtConta.text,0);
  lancamento.cod_plano:= StrToIntDef(edtCodPlano.text,0);
  lancamento.data_mvto:= dVencimento;
  lancamento.descricao:= cParcela+' - '+edtDescLcto.text;
  if plano.tipo = 'C' then
     lancamento.valor    := edtValor.Value
  else
    lancamento.valor     := (edtValor.Value*-1);
